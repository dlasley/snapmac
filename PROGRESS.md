...
- [ ] Pluggable storage backends: abstract push layer so users can push snapshots to GitHub, S3, GDrive, Dropbox, Azure, FTP, etc. Plan: keep `snapshot.sh` in bash; rewrite push layer in Python with a proper ABC that backend modules implement. Python 3 is already a de facto dependency (used inline for JSON parsing). Ship `git` and `s3` (boto3) as reference backends. Users implement a Python class for custom storage. A single var or param (`--versioned` / `SNAPMAC_VERSIONED=true`) will control whether each backend writes timestamped new files or overwrites a single static endpoint — consistent behavior across all storage types.
- [ ] Add config-dirs module (opt-in, default-off) to capture dotfiles and app-specific config like `~/.claude`, `~/.config/gh`, VS Code settings, etc. Design: `SNAPMAC_CONFIG_PATHS` allowlist + `SNAPMAC_CONFIG_EXCLUDE_PATTERNS` in `.env`; module tars allowlisted paths into `snapshot_dir/config.tar.gz`; `restore.sh` extracts them back. Ship a curated starter list of safe paths. Concerns to address: (1) secrets — many config dirs contain API keys/tokens (SSH keys, AWS creds, session cookies); flag for user review, (2) noise — exclude caches/logs/lock files, (3) curation — no canonical list so user must decide, (4) restore ordering — config restore runs last, after tools are installed. Default-off because the secrets risk is real.
